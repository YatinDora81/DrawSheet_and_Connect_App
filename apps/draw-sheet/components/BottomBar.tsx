...
function BottomBar({ selectedColor = Colors[0]?.color_code }: { selectedColor?: string }) {
  return (
    <div className=' absolute bg-[#26262A]/70 left-[50%] bottom-[2vh] translate-x-[-50%] flex   justify-evenly items-center w-[38rem] h-[3rem] rounded-2xl gap-1 border border-gray-700'>
      ...
      <div className=' flex justify-center items-center h-full w-[55%] gap-2'>
        ...
        <div className=' flex justify-start items-center h-full gap-[5px]'>
          {
            Colors.map((c) => <div key={c.color_name} className={` h-[1.6rem] hover:scale-[1.1] transition-all duration-300 ease-in-out rounded-full aspect-square ${selectedColor === c.color_code && 'border-3  border-zinc-500 scale-[1.1]'}`} style={{ backgroundColor: c.color_code }}></div>)
          }
