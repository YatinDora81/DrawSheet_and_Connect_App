            ...
            <div className='min-h-[32rem] w-[30rem] flex flex-col ' >
                ...
                <div className=' min-h-[28rem] w-full flex justify-center items-center'>
                    <div className=' min-h-[24rem] w-[95%] border-[0.1px] rounded-lg border-zinc-800/70 text-white flex flex-col  gap-5 items-start' style={{paddingBlock : "1.1rem" , paddingInline : "1.4rem"}}>
                        ...
                        <div className=' min-h-[19.5rem] w-full flex flex-col gap-5'>
                            {
                                !isSignIn && <div className=' flex flex-col justify-center items-start gap-2'>
                                <label htmlFor='draw-name' className=' text-[0.95rem] font-semibold'>Jhon doe</label>
                                <input id='draw-name' type='text' className=' border border-zinc-800/70 w-full text-[0.91rem] font-normal  rounded-lg' style={{paddingInline : "0.9rem" , paddingBlock : "0.5rem"}} placeholder='name' />
                            </div>
                            }
                            <div className=' flex flex-col justify-center items-start gap-2'>
                                <label htmlFor='draw-email' className=' text-[0.95rem] font-semibold'>Email</label>
                                <input id='draw-email' type='email' className=' border border-zinc-800/70 w-full text-[0.91rem] font-normal  rounded-lg' style={{paddingInline : "0.9rem" , paddingBlock : "0.5rem"}} placeholder='name@example.com' />
                            </div>
                            <div className=' flex flex-col justify-center items-start gap-2'>
                                ...
                                <div className=' w-full relative'>
                                    <input id='draw-pass' type={showPassword ? "text" : "password"}  className=' border border-zinc-800/70 w-full text-[0.91rem] font-normal  rounded-lg' style={{paddingInline : "0.9rem" , paddingRight : "2.2rem" , paddingBlock : "0.5rem"}} placeholder="**************" />
                                    {!showPassword ? <FaEye className=' absolute right-[0.8rem] cursor-pointer top-[0.7rem]' onClick={()=>setShowPassword(true)} /> : <FaEyeSlash className=' absolute right-[0.8rem] cursor-pointer top-[0.7rem]' onClick={()=>setShowPassword(false)} />}
                                </div>
                            </div>
                            {
                                !isSignIn && <div className=' flex flex-col justify-center items-start gap-2'>
                                ...
                                <div className=' w-full relative'>
                                    <input id='draw-conf-pass' type={showPassword2 ? "text" : "password"}  className=' border border-zinc-800/70 w-full text-[0.91rem] font-normal  rounded-lg' style={{paddingInline : "0.9rem" , paddingRight : "2.2rem" , paddingBlock : "0.5rem"}} placeholder="**************" />
                                    {!showPassword2 ? <FaEye className=' absolute right-[0.8rem] cursor-pointer top-[0.7rem]' onClick={()=>setShowPassword2(true)} /> : <FaEyeSlash className=' absolute right-[0.8rem] cursor-pointer top-[0.7rem]' onClick={()=>setShowPassword2(false)} />}
                                </div>
                            </div>
                            }

                            <div className=' text-[0.75rem] text-red-500' style={{marginBlock : "-0.9rem"}}>* {"this is an error!!!!!!!!!"}</div>
                            <button className=' border border-zinc-800/70 w-full text-[0.91rem] rounded-lg bg-blue-500 font-bold' style={{paddingInline : "0.9rem" , paddingBlock : "0.5rem"}} >Log in</button>
