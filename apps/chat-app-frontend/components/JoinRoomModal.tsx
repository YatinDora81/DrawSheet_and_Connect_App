...
const JoinNewRoomModal = ({ showModal, setShowModal }: { showModal: boolean, setShowModal: (value: boolean) => void }) => {
    ...
    return (
        <div onClick={(e)=>{e.stopPropagation()}} className=' relative text-white w-[45%] min-h-[35%] border border-zinc-700 bg-zinc-900 rounded-2xl py-4 px-6 flex flex-col items-start justify-evenly'>
            ...
            <div className=' w-full flex flex-col gap-2'>
                ...
                <button className=' w-full bg-blue-600 rounded-sm  transition-all duration-200 hover:bg-blue-700 cursor-pointer h-10 text-xl'>Join Room</button>
            </div>
        </div>
    )
}
