...
const JoinNewRoomModal = ({ showModal, setShowModal }: { showModal: number, setShowModal: (value: number) => void }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await authenticatedFetch(JOIN_NEW_ROOM_URL, { method: "POST", body: JSON.stringify({ "roomJoinCode": inputRef.current.value }), credentials: "include" });
    ...
    return (
        <div onClick={(e) => { e.stopPropagation() }} className=' relative text-white w-[90%] sm:w-[45%] min-h-[35%] border border-zinc-700 bg-zinc-900 rounded-2xl py-4 px-6 flex flex-col items-start justify-evenly'>
...
export default JoinNewRoomModal
